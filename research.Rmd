...
output: 
    html_document:
        ...
        toc_float: TRUE
runtime: shiny
description: "Species Delineation from Herbarium Sheet Samples. Unsupervised learning research for developing workflows in Paleo-Botony."
---
...
## Introduction
The goal of this research is to use machine learning to capture and quantify the intuition that botanists have about when considering the further speciation of a particular plant. Specifically we will be applying a convolutional variational autoencoder to a few hundred
vitis arizonica specimen. Building this network will allow us to see if there is any sort of clustering among the samples, and if there is we might also be able to find which samples show the greatest difference in morphology. Doing so guide how we choose which samples get sent to undergo genetic sequencing. 
